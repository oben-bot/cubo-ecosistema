...
- 🔜 **E2 — Cubo Manager: verificar y conectar a la Biblioteca real** (brief: `docs/fases/FASE_E2_CUBO_MANAGER.md`). **Parcialmente hecho:** arena.ai se quedó sin créditos antes de tomar esta fase, así que Claude construyó directamente (con verificación propia) lo que no depende de ejecutar la app: bridge HTTP real a la Biblioteca, contraseña con hash real (`scrypt`), recuperación por correo desactivada honestamente, y los 5 duplicados `.js`/`.jsx` resueltos (se confirmó por análisis estático cuál versión cargaba realmente cada uno). PR #2 en `cubo-arena`, fusionado y trasladado aquí. Verificado con `node --check` y compilación `@babel/preset-react`; **no** se corrió la app end-to-end (sin Electron/pantalla disponible). Detalle completo en `BITACORA.md`.
  - **Sigue pendiente de E2:** endurecer los canales IPC genéricos `database:query/run/get`, todavía usados en 6 módulos (Clientes, Ventas, Marketing, Producción, Ajustes, Cotizaciones).
  - **El dueño** sigue pendiente de correr Cubo Manager de verdad en su PC (Windows), contra una Biblioteca levantada, para confirmar en vivo lo que aquí solo se verificó estáticamente.
...
| **E2** | Verificar Cubo Manager y conectarlo a la Biblioteca | Claude (código, por falta de créditos de arena.ai) + dueño (probar en su PC) | 🔜 **Parcial** — bridge HTTP real, contraseña con hash, recuperación desactivada honestamente y duplicados `.js`/`.jsx` resueltos ✅ (verificado estáticamente); falta endurecer IPC genérico en 6 módulos y probar en vivo en la PC del taller |
