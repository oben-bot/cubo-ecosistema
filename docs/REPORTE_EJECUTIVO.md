...
**Estado ahora mismo (26 de septiembre de 2026):**
...
- ✅ **E3 — Costeo:** hecho y verificado. Pantalla completa (configuración de máquina, catálogo de materiales e insumos, calculadora) sobre el backend que ya existía. Claude probó el motor de cálculo con datos propios y confirmó a mano que el resultado coincide (109.62 con los datos de prueba), y que recalcular no duplica costos (corrección de idempotencia). Compila limpio. Detalle en `BITACORA.md`.
- ✅ **E4 — Taller (texto, cajas, llaveros):** hecho y verificado. `apps/taller`: letras convertidas a contornos y soldadas en una sola pieza recortable (`opentype.js` + `clipper2-js`), probado en 5 tipografías con licencia libre (SIL OFL); cajas en plancha; llaveros circular/rectangular/hueso con texto soldado. Un diseño pasa de generado → bandeja → guardado como original en la Biblioteca real, probado de punta a punta con los servicios corriendo de verdad.
- ✅ **E5 — Catálogo + Web (`apps/catalogo-web`, puerto 7103):** hecho y verificado. Publicar un producto real de la Biblioteca (verificado contra ella) y verlo en `GET /productos`, público y sin llave — probado en vivo.
- ✅ **E6 — Venta de archivos y cola de entregas:** hecho y verificado. `POST /ventas` + `POST /entregas/:id/enviar`: nube+automática entrega de inmediato; con ubicación "pc", la disponibilidad de la Biblioteca se usa como señal de "PC encendida" — probado apagándola de verdad (da 502 con aviso claro) y reencendiéndola (el reintento entrega bien, 200). n8n de verdad (confirmación de pago, avisos por WhatsApp) queda pendiente, no bloquea lo ya construido.
- 🔜 **E7 — Conector de asistente** (brief: `docs/fases/FASE_E7_ASISTENTE.md`). Exponer en Cubo Manager las herramientas que cualquier asistente de IA pueda llamar (buscar diseño, cotizar, crear pedido, registrar venta, preparar publicación), sin integrar ningún asistente concreto todavía — ni Hermes ni la Wallet, solo el enchufe.
- **Pendiente:** E8 — empaquetado para vender el ecosistema (idiomas es/en/zh, licencias, instalador de Windows). Detalle: sección 10 y `BITACORA.md`.
...
| **E3** | Costeo y precios con pantalla; alertas de stock | arena.ai | ✅ **Hecho y verificado** — configuración de máquina, materiales, insumos y calculadora completa; cotizar un producto de punta a punta desde la interfaz |
| **E4** | Taller v1: texto, cajas, llaveros; bandeja | arena.ai (verificado por Claude) | ✅ **Hecho y verificado** — texto soldado en 5 tipografías libres (opentype.js + clipper2-js), cajas en plancha, llaveros circular/rectangular/hueso con texto; un diseño pasa de constructor a la Biblioteca real de punta a punta |
| **E5** | Catálogo + Web unidos; flujo del catálogo | arena.ai (verificado por Claude) | ✅ **Hecho y verificado** — `apps/catalogo-web`: producto de la Biblioteca real publicado y visible en `GET /productos` público, sin llave |
| **E6** | Venta de archivos: cola de entregas, pagos | arena.ai (verificado por Claude) | ✅ **Hecho y verificado** — nube+automática entrega de inmediato; con "PC apagada" (Biblioteca no disponible) da 502 con aviso claro, y al "encenderla" el reintento entrega bien. n8n de verdad (pagos, avisos) queda pendiente, no bloquea |
