...
const MARCA_GENERICA: Marca = {
  nombre: 'Catálogo El Cubo',
  logo: null,
  colores: { primario: '#8b5e34', fondo: '#ffffff', texto: '#2b2118' },
  idioma: null,
  enlaces: {},
};
...
function idiomaDe(valor: string | undefined, porDefecto: Idioma, clave: string): Idioma {
  ...
  if (!IDIOMAS_DISPONIBLES.includes(normalizado)) {
    throw new ErrorConfiguracion('error.config_opciones_invalidas', { variable: clave, opciones: IDIOMAS_DISPONIBLES.join(', ') });
  }
  return normalizado;
}
...
export function leerMarca(ruta: string | null): { marca: Marca; rutaUsada: string | null } {
  ...
  for (const candidato of candidatos) {
    ...
    try {
      ...
      throw new ErrorConfiguracion('error.marca_invalida', { ruta: candidato, detalle: (error as Error).message });
    }
    ...
    const marca: Marca = {
      ...
      idioma: typeof datos.idioma === 'string' && IDIOMAS_DISPONIBLES.includes(datos.idioma as Idioma) ? (datos.idioma as Idioma) : null,
      enlaces: typeof datos.enlaces === 'object' && datos.enlaces !== null ? Object.fromEntries(Object.entries(datos.enlaces as Record<string, unknown>).filter(([, v]) => typeof v === 'string') as Array<[string, string]>) : {},
    };
...
export function cargarConfiguracion(opciones: OpcionesCargarConfig = {}): Configuracion {
  ...
  const rutaEnv = opciones.archivoEnv === undefined ? path.resolve(cwd, '.env') : opciones.archivoEnv === null ? null : path.resolve(opciones.archivoEnv);
  ...
  for (const [clave, valor] of Object.entries(baseEnv)) {
    if (clave.startsWith('CATALOGO_') && valor !== undefined && valor !== '') vars[clave] = valor;
  }

  const rutaCatalogo = path.resolve(cwd, vars.CATALOGO_RUTA ?? 'datos');
  const { marca, rutaUsada } = leerMarca(vars.CATALOGO_MARCA ?? null);
  const idioma = idiomaDe(vars.CATALOGO_IDIOMA, marca.idioma ?? IDIOMA_POR_DEFECTO, 'CATALOGO_IDIOMA');

  const config: Configuracion = {
    rutaCatalogo,
    rutaBaseDatos: path.join(rutaCatalogo, 'catalogo.db'),
    rutaLlave: path.join(rutaCatalogo, 'cubo.key'),
    host: vars.CATALOGO_HOST ?? HOST_POR_DEFECTO,
    puerto: entero(vars.CATALOGO_PUERTO, PUERTO_POR_DEFECTO, 'CATALOGO_PUERTO', 1, 65535),
    idioma,
    limiteSubidaBytes: entero(vars.CATALOGO_LIMITE_SUBIDA_MB, LIMITE_SUBIDA_MB_POR_DEFECTO, 'CATALOGO_LIMITE_SUBIDA_MB', 1, 10240) * 1024 * 1024,
    llave: vars.CATALOGO_LLAVE ?? null,
    marca,
    rutaMarca: rutaUsada,
    archivoEntorno,
    bibliotecaUrl: vars.CATALOGO_BIBLIOTECA_URL ?? 'http://127.0.0.1:7101',
    bibliotecaLlave: vars.CATALOGO_BIBLIOTECA_LLAVE ?? null,
  };
