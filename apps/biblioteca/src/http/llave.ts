 ...
 *
 * Se genera en el primer arranque y se guarda junto a los datos de la instalacion,
 * con permisos de solo lectura para el usuario actual. Nunca se escribe en el codigo
 * ni se sube a Git.
 */
...
export function cargarOGenerarLlave(rutaLlave: string, llaveDeConfiguracion: string | null): LlaveLocal {
  ...
  try {
    ...
    try {
      ...
      // En algunos sistemas de archivos no se pueden ajustar permisos.
    }
    return { llave, generada: true, ruta: rutaLlave };
  } catch {
    // Si no se puede guardar, la llave vale solo para esta ejecucion.
    return { llave, generada: true, ruta: null };
  }
}

/** Compara en tiempo constante para no filtrar la llave por tiempo de respuesta. */
